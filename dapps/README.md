...
Run the TypeScript code of one file within the terminal using `bun` from the root directory.

```bash
bun run dapps/file-path/script.ts
```
...
🪙 🔍 Let's now inspect the metadata of the first Digital Asset. In the file `digital-assets/extract-asset-data.ts`, adjust the address here as follow:
...
```bash
bun run dapps/digital-assets/extract-asset-data.ts
```
...
- [`universal-factory`](./universal-factory): Universal Factory related scripts
...
Please check the versions used in the main [`package.json`](../package.json)
